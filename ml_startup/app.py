...
def  create_bucket(name: str):
    try:
        ...
        s3.Bucket(name).create(CreateBucketConfiguration=location)
...
def list_objects(name: str):
    try:
        ...
        for version in objects["Versions"]:
            key = version["Key"]
            version_id = verison["VersionId"]
            response = client.get_object_tagging(
                Bucket = name, Key=key, VersionId = version_id
            )
            typer.echo(
                f"Key: {key} VerisionId: {version_id} IsLatest: {version['IsLatest']} LastModified: {version['LastModified']} Tag: {response['TagSet']}"
            )
    except Exception as e:
        typer.echo(f"error : {e}")
